...
async fn async_main() -> Result<(), ValidationError> {
    ...
    match args.get(1).map(|s| s.as_str()) {
        ...
        _ => {
            ...
            println!("==================================================");
            println!();
            println!("World's most comprehensive Rust compilation pipeline analysis toolkit.");
            println!("Extract semantic analysis, project structure, and LLVM IR generation data");
            println!("from Rust codebases for machine learning and compiler research.");
            println!();
            println!("📊 ACHIEVEMENTS:");
            println!("  • 1.4+ Million Records: Largest Rust analysis dataset ever created");
            println!("  • Self-Referential Analysis: Tools analyzing their own codebases");
            println!("  • Complete Pipeline: Source code → LLVM IR generation");
            println!("  • HuggingFace Ready: Available at huggingface.co/datasets/introspector/rust");
            println!();
            println!("🚀 QUICK START:");
            println!("  # Complete pipeline analysis (recommended)");
            println!("  cargo run --bin hf-validator -- analyze-rust-to-ir /path/to/rust/project output-dataset");
            println!();
            ...
            println!("  git clone https://github.com/rust-lang/rust-analyzer.git");
            println!("  cargo run --bin hf-validator -- analyze-rust-to-ir rust-analyzer rust-analyzer-dataset");
            println!();
            println!("  # Analyze Rust compiler (835K records)");
            println!("  git clone https://github.com/rust-lang/rust.git");
            println!("  cargo run --bin hf-validator -- generate-hf-dataset rust/compiler rustc-dataset");
            println!();
            println!("  # Quick test on simple file");
            println!("  echo 'fn main() {{ println!(\"Hello!\"); }}' > test.rs");
            println!("  cargo run --bin hf-validator -- analyze-rust-to-ir test.rs test-dataset");
            println!();
            println!("📊 OUTPUT FORMAT:");
            println!("  All commands generate Apache Parquet files optimized for ML workflows:");
            println!("  output-dataset/");
            println!("  ├── semantic/           - Rust semantic analysis data");
            println!("  ├── cargo/              - Project structure and metadata");
            println!("  ├── llvm-ir/            - LLVM IR generation across optimization levels");
            println!("  └── README.md           - Comprehensive documentation");
            println!();
            println!("🎯 USE CASES:");
            println!("  • ML Research: Train code understanding and performance prediction models");
            println!("  • Compiler Research: Analyze optimization patterns and type system behavior");
            println!("  • Tool Development: Build better Rust development tools and IDEs");
            println!("  • Education: Understand compilation processes and language internals");
            println!();
            println!("📚 MORE INFO:");
            println!("  • Documentation: https://github.com/solfunmeme/hf-dataset-validator-rust");
            println!("  • Dataset: https://huggingface.co/datasets/introspector/rust");
            println!("  • License: AGPL-3.0");
            println!();
            println!("🚀 Ready to revolutionize Rust analysis and ML-powered development tools!");
